...
int64_t bts_simd(tree_t *tree, relation_t *rel, void *output) {
  ...
  __m512i v_offset = _mm512_set1_epi64(0), v_addr_offset = _mm512_set1_epi64(0),
          ...
          v_tuple_key = _mm512_set1_epi64(0), v_base_offset, v_node_key,
          v_cell_hash, v_tree_pos = _mm512_set1_epi64(tree->first_node),
          ...
          v_tree_addr = _mm512_set1_epi64(tree->first_node),
          ...
          v_lnext_off = _mm512_set1_epi64(16), v_tuple_payload, v_node_payload,
          v_rnext_off = _mm512_set1_epi64(24),
 ...
 */
    ...
    v_tuple_key = _mm512_mask_i64gather_epi64(
        v_tuple_key, m_new_cells, v_addr_offset, ((void *)rel->tuples), 1);
    v_tuple_payload = _mm512_mask_i64gather_epi64(
        v_tuple_payload, m_new_cells,
        _mm512_add_epi64(v_addr_offset, v_word_size), ((void *)rel->tuples), 1);

    // step 3: load new nodes from tree
    v_tree_pos = _mm512_mask_blend_epi64(m_new_cells, v_tree_pos, v_tree_addr);

    /////////////////// random access
    v_node_key = _mm512_mask_i64gather_epi64(v_neg_one512, m_have_tuple,
                                             v_tree_pos, 0, 1);
    ///// step 4: compare;
    ...
    v_node_payload = _mm512_mask_i64gather_epi64(
        v_neg_one512, m_match, _mm512_add_epi64(v_tree_pos, v_payload_off), 0,
        1);
    ...
    v_next_addr =
        _mm512_mask_add_epi64(v_tree_pos, m_less, v_tree_pos, v_lnext_off);
    v_next_addr =
        _mm512_mask_add_epi64(v_next_addr, m_more, v_tree_pos, v_rnext_off);

    v_tree_pos = _mm512_mask_i64gather_epi64(v_tree_pos, m_have_tuple,
                                             v_next_addr, 0, 1);
    m_have_tuple = _mm512_kand(_mm512_cmpneq_epi64_mask(v_tree_pos, v_zero512),
                               m_have_tuple);
  }
  return matches;
}
...
// the tail depends on the number of joins and tuples in each bucket
      ...
      case 0: {
        v_node_key = _mm512_mask_i64gather_epi64(
        ...
        v_next_addr = _mm512_mask_add_epi64(state[k].ht_off, m_less,
                                            state[k].ht_off, v_lnext_off);
        v_next_addr = _mm512_mask_add_epi64(v_next_addr, m_more,
                                            state[k].ht_off, v_rnext_off);

        // update next
        state[k].ht_off = _mm512_mask_i64gather_epi64(
            state[k].ht_off, state[k].m_have_tuple, v_next_addr, 0, 1);
        state[k].m_have_tuple =
...
// the tail depends on the number of joins and tuples in each bucket
        ...
        ///// step 3: load new values from hash tables;
        state[k].ht_off =
            _mm512_mask_blend_epi64(m_new_cells, state[k].ht_off, v_tree_addr);
        state[k].stage = 0;
#if KNL
        _mm512_mask_prefetch_i64gather_pd(
            state[k].ht_off, state[k].m_have_tuple, 0, 1, _MM_HINT_T0);
...
int64_t bts_smv(tree_t *tree, relation_t *rel, void *output) {
  ...
  __mmask8 m_match = 0, m_new_cells = -1, m_valid_bucket = 0,
           mask[VECTOR_SCALE + 1], m_less, m_more;
  __m512i v_offset = _mm512_set1_epi64(0), v_addr_offset = _mm512_set1_epi64(0),
          v_base_offset_upper =
              _mm512_set1_epi64(rel->num_tuples * sizeof(tuple_t)),
          v_tuple_key = _mm512_set1_epi64(0), v_base_offset, v_node_key,
