#ifndef _SMV_PIPELINE
#define _SMV_PIPELINE

#include "prefetch.h"
#include "tuple_buffer.h"
#define WORDSIZE 8
// target for 8B keys and 8B payload
int64_t pipeline_simd(hashtable_t *ht, relation_t *rel, void *output) {
  int64_t matches = 0;
  int32_t new_add = 0;
...
// the tail depends on the number of joins and tuples in each bucket
    ...
    v_left_payload = _mm512_mask_i64gather_epi64(
        v_left_payload, m_new_cells,
        _mm512_add_epi64(v_addr_offset, v_word_size), ((void *)rel->tuples), 1);

    //// step 2.5: filter
    v_A = _mm512_mullo_epi64(v_tuple_cell, SIMD_A);
    m_match = _mm512_cmpge_epi64_mask(v_A, SIMD_B);
    m_new_cells = _mm512_kand(m_new_cells, m_match);
    m_have_tuple = _mm512_kand(m_have_tuple, m_match);
...
int64_t pipeline_simd_amac(hashtable_t *ht, relation_t *rel, void *output) {
  ...
  __m512i v_offset = _mm512_set1_epi64(0),
          ...
          v_zero512 = _mm512_set1_epi64(0), v_A,
          v_write_index = _mm512_set1_epi64(0),
          v_ht_addr = _mm512_set1_epi64(ht->buckets),
          v_word_size = _mm512_set1_epi64(WORDSIZE),
          v_tuple_size = _mm512_set1_epi64(sizeof(tuple_t)),
...
int64_t pipeline_simd_amac_raw(hashtable_t *ht, relation_t *rel, void *output) {
  int64_t matches = 0;
  int32_t new_add = 0, k = 0, done = 0;
  __mmask8 m_match = 0, m_new_cells = -1, m_valid_bucket = 0;
  __m512i v_offset = _mm512_set1_epi64(0),
...
int64_t pipeline_simd_amac_compact(hashtable_t *ht, relation_t *rel,
                                   void *output) {
  int64_t matches = 0;
...
int64_t pipeline_smv(hashtable_t *ht, relation_t *rel, void *output) {
  int64_t matches = 0;
  int32_t new_add = 0, k = 0, done = 0, num, num_temp;
  __mmask8 m_match = 0, m_new_cells = -1, m_valid_bucket = 0,
  ...
  __m512i v_offset = _mm512_set1_epi64(0),
          ...
          v_tuple_size = _mm512_set1_epi64(sizeof(tuple_t)),
          v_bucket_size = _mm512_set1_epi64(sizeof(bucket_t)),
          v_next_off = _mm512_set1_epi64(8), v_right_payload,
          v_payload_off = _mm512_set1_epi64(24);
  chainedtuplebuffer_t *chainedbuf = (chainedtuplebuffer_t *)output;
  tuple_t *join_res = NULL;
  __attribute__((aligned(64))) uint64_t cur_offset = 0, base_off[16], *ht_pos;
...
int64_t pipeline_simd_amac_compact2(hashtable_t *ht, relation_t *rel,
                                    void *output) {
  int64_t matches = 0;
  int32_t new_add = 0, k = 0, done = 0, num, num_temp;
  __mmask8 m_match = 0, m_new_cells = -1, m_valid_bucket = 0,
           mask[VECTOR_SCALE + 1], flag, m_have_tuple;
  __m512i v_offset = _mm512_set1_epi64(0),
          v_base_offset_upper =
              _mm512_set1_epi64(rel->num_tuples * sizeof(tuple_t)),
          v_base_offset, v_left_size = _mm512_set1_epi64(8),
...
// the tail depends on the number of joins and tuples in each bucket
      ...
      case 0: {
...
#endif
        {
          ...
          // compress v
          state_temp[0].ht_off = _mm512_maskz_compress_epi64(
              state[k].m_have_tuple, state[k].ht_off);
          state_temp[0].key =
              _mm512_maskz_compress_epi64(state[k].m_have_tuple, state[k].key);
          ...
          state_temp[1].ht_off = _mm512_mask_expand_epi64(
              ...
              _mm512_knot(state[SIMDStateSize].m_have_tuple),
              state_temp[0].ht_off);
          state_temp[1].key = _mm512_mask_expand_epi64(
              state[SIMDStateSize].key,
              _mm512_knot(state[SIMDStateSize].m_have_tuple),
              state_temp[0].key);
          ...
          {  // merge the results of the two cases
            state[k].ht_off = _mm512_mask_mov_epi64(state[k].ht_off, flag,
                                                    state_temp[0].ht_off);
            state[k].key =
            ...
            state[SIMDStateSize].m_have_tuple = _mm512_kor(
                _mm512_kand(mask[num + num_temp], flag),
                _mm512_kand(
                    (state[SIMDStateSize].m_have_tuple >> (VECTOR_SCALE - num)),
                    _mm512_knot(flag)));
...
int64_t pipeline_simd_gp(hashtable_t *ht, relation_t *rel, void *output) {
  int64_t matches = 0;
